...
![](goal_locations_ice_rink.png){width=60% fig-align="center"}
...
We get the angle of the shot (`Shot_Angle`), the Meridian distance, which is the distance between the shot location and the meridian (royal road) `d_m`, and the shot distance (distance between the shot location and the goal) (`d_trig` and `d_euclidean`).
