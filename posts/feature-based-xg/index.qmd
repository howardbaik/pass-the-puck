...
&emsp; &nbsp; Later, we use this time interval to calculate velocity:
...
4. `MAX_SPEED_MPH = 30.0` is the maximum speed (in mph) we consider valid for our model. The maximum skating speed for an NHL player is just under 25 mph, so a speed above 30 mph is determined a tracking glitch. 
...
![](rink_coords_meridian.png){width=70% fig-align="center"}
...
puck crosses it when its Y coordinate changes sign which forces the goalie across and raises shot danger. Features 1 and 2 are only defined when the crossing happened within 5 s before the shot.
