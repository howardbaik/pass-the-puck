...
With feature engineering complete, we consolidate all the features above into a single dataset by joining them on `shot_id`. 
...
Section 2.4.2 of the thesis lays out how to train an XGBoost model on the dataset. As the thesis emphasizes, the goal is not to classify each shot as a goal or non-goal, but to estimate its probability of becoming a goal. Since the output is a probability, the model is evaluated with four metrics: binary cross-entropy loss and the Brier score, which directly score probabilistic predictions, along with the area under the receiver operating characteristic curve (AUC) and the estimated calibration error (ECE).

We follow the hyperparameters laid out in the thesis: the maximum depth of a tree is 4, the number of trees is 125, and the learning rate is 0.075.
